...
import { Monitor, Play, Square, Zap, Settings2, BrainCircuit } from "lucide-react";
...
export default function Desktop() {
  ...
  const startCapture = async () => {
    if (!selectedModel) {
      alert("请先选择一个 ONNX 模型");
      return;
    }
    try {
      await invoke("start_capture", { modelPath: selectedModel, confThreshold: confThreshold });
      await refreshState();
    } catch (e) {
      console.error(e);
    }
          ...
          <div className="bg-black rounded-2xl overflow-hidden aspect-video relative flex items-center justify-center shadow-lg">
            {frame ? (
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 font-medium">未开始捕获</p>
                <p className="text-xs text-gray-600 mt-1">选择模型后点击开始按钮启动屏幕捕获</p>
              </div>
            )}
